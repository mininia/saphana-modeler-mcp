...
- **Secure defaults (fail-closed)**:
  ...
  - **Authentication**: set `MCP_HTTP_TOKEN` (≥16 chars) to enforce bearer-token validation on every request (missing/mismatched → 401); unset = the loopback binding is the access boundary. **External listening without a token is refused at startup** (a blocking deployment invariant, see above); if you really need it, set `MCP_HTTP_ALLOW_ANONYMOUS=true`, and the startup log will warn on every boot
...
>
> **Write-boundary preflight (validated at startup)**: on boot the server prints the effective writable-package scope — `info` with the allowlist when configured, and a `warn` when it is empty (= unrestricted) while write tools are exposed to clients. A silent downgrade ("assumed a boundary was set, everything was actually writable") thus surfaces at startup rather than at the first write. Malformed entries in `HANA_WRITE_PACKAGES` (`ZDEMO.`, `ZDE MO`, `*`, …) now fail at config load with a fix hint instead of being silently kept as a prefix that never matches (which either rejected every write or made the boundary a no-op).
> Note the effective value depends on the launch source: a stdio server launched via mcp.json takes that file's env, and when the credential trio is present the repository `.env` is skipped entirely — three config locations on one machine can give three different answers.
>
...
>
> Judging uses the config the server already loaded (no env re-parsing), so there is no "value used for judging ≠ value used at runtime". Cross-package read references are currently **reported but not blocked** (there is no "readable packages" config item, and adding an allowlist would break existing cross-package join usage). The service-layer `assertWritePackageAllowed` remains as defence in depth (scripts and internal calls bypass the tool layer) and reuses the same rule. Self-check before a task: `npm run preflight -- <package...> [--tool <name>] [--schema <name>]`, exit code `0` = may execute / `1` = blocked / `2` = usage error; for local mode add `--from <mcp.json>` to judge against the config the client actually injects (a different source from the repository `.env`).
>
> **Deployment invariants (refused at startup)**: configs that are individually valid but unsafe in combination are rejected by the startup self-check rather than merely warned about (mirroring tableau-mcp, which throws in its Config constructor). Current rules: (1) HTTP bound to a non-loopback address with no Bearer token → **startup refused** (unless `MCP_HTTP_ALLOW_ANONYMOUS=true` is set explicitly); (2) `HANA_TLS=false` plaintext, (3) encrypted but `HANA_SSL_VALIDATE=false` (no MITM protection) → warned and allowed, but visible in the startup log every time.
